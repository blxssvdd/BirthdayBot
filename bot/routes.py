...
from .calendar import get_years_kb, get_months_kb, get_days_kb, get_confirm_kb
...
async def confirm_birthday(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Сохраняем дату в базу
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('INSERT OR REPLACE INTO users (user_id, birthday) VALUES (?, ?)', (callback_query.from_user.id, birthday))
    conn.commit()
    conn.close()
...
async def process_timezone(message: Message, state: FSMContext):
    try:
        ...
        if not tz:
            ...
            return
        # Сохраняем часовой пояс сразу
        conn = sqlite3.connect('users.db')
        c = conn.cursor()
        c.execute('UPDATE users SET timezone = ? WHERE user_id = ?', (tz, message.from_user.id))
        conn.commit()
        conn.close()
...
async def days_until_birthday(message: Message):
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('SELECT birthday, timezone FROM users WHERE user_id = ?', (message.from_user.id,))
    row = c.fetchone()
    conn.close()
    if not row or not row[0] or not row[1]:
        await message.answer('Сначала завершите регистрацию!')
        return
    birthday = datetime.strptime(row[0], '%Y-%m-%d').date()
    tz = pytz.timezone(row[1])
...
async def set_new_birthday(message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('UPDATE users SET birthday = ? WHERE user_id = ?', (birthday.isoformat(), message.from_user.id))
    conn.commit()
    conn.close()
...
async def confirm_timezone_change_handler(callback: CallbackQuery, state: FSMContext):
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('UPDATE users SET timezone = ? WHERE user_id = ?', (tz, callback.from_user.id))
    conn.commit()
    conn.close()
...
# Fallback-хендлер для отладки FSM
@router.message()
...
async def calendar_confirm_handler(callback: CallbackQuery, state: FSMContext):
    date_str = callback.data.split(':')[2]  # '16.10.2008'
    ...
    if current_state == SettingsState.waiting_for_new_birthday.state:
        ...
        conn = sqlite3.connect('users.db')
        c = conn.cursor()
        c.execute('UPDATE users SET birthday = ? WHERE user_id = ?', (iso_date, callback.from_user.id))
        conn.commit()
        conn.close()
        ...
        # Регистрация
        await state.update_data(birthday=iso_date)
        conn = sqlite3.connect('users.db')
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO users (user_id, birthday) VALUES (?, ?)', (callback.from_user.id, iso_date))
        conn.commit()
        conn.close()
        await callback.message.edit_text(
            'Теперь отправьте ваш город или поделитесь геолокацией для определения часового пояса.'
